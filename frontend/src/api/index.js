...
// Get compatibility data between team and candidate (includes score AND summary)
export const getCompatibilityData = async (teamId = 1, candidateId = 1) => {
  if (USE_MOCK_DATA) {
    ...
    return new Promise(resolve => {
      ...
      // Include summary from mockSummary
      modifiedData.summary = mockSummary.insights;
  ...
  try {
    ...
    // This is now our single API call that returns all compatibility data including the summary
    const response = await api.get(`/compatibility/${candidateId}/${teamId}`);
    return response.data;
  } catch (error) {
    console.error(`Error fetching compatibility data for team ${teamId}, candidate ${candidateId}:`, error);
    throw error;
  }
};
...
export const getCompatibilityScore = async (teamId = 1, candidateId = 1) => {
  console.warn('getCompatibilityScore is deprecated. Use getCompatibilityData instead.');
  const data = await getCompatibilityData(teamId, candidateId);
  return data;
};

export const getCompatibilitySummary = async (teamId = 1, candidateId = 1) => {
  console.warn('getCompatibilitySummary is deprecated. Use getCompatibilityData instead.');
  const data = await getCompatibilityData(teamId, candidateId);
  return { insights: data.summary };
};